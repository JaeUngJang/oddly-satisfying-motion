...
# Wow Unit: Reward Burst (reward-burst)
...
Tags: surface=button, trigger=complete, intent=reward  · SwiftUI · minimum iOS 16
Budgets: touch→haptic ≤ 50 ms · touch→first frame ≤ 85 ms · haptic ≤ 150 ms
Raw files (same bytes as sections A and B below): /files/WowCore.swift · /files/WowRewardBurst.swift

## Steps
1. If `WowCore.swift` is not in the app target yet, create it with the content in section A. Never edit it.
2. Create `WowRewardBurst.swift` with the content in section B. Never edit it.
3. Use it: Button("Claim") { … }.wowRewardBurst(trigger: claimed)
...
## States
- idle · motion: no overlay mounted · haptic: none
- bursting · motion: 12–40 pieces launch from the button top edge, closed-form drag + gravity, ≤1 s, fade over the last 30% · haptic: .burst on the first frame, before any visual work
- done · motion: overlay removed when the last piece dies · haptic: none
- reduced-motion · motion: one glow pulse from the button, scale 1→1.6, opacity 0.5→0 over 400 ms · haptic: .burst unchanged
