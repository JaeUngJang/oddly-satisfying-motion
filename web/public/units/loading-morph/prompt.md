...
# Wow Unit: Loading Morph (loading-morph)
...
Tags: surface=button, trigger=complete, intent=convert  · SwiftUI · minimum iOS 16
Budgets: touch→haptic ≤ 50 ms · touch→first frame ≤ 85 ms · haptic ≤ 120 ms
Raw files (same bytes as sections A and B below): /files/WowCore.swift · /files/WowLoadingMorph.swift

## Steps
1. If `WowCore.swift` is not in the app target yet, create it with the content in section A. Never edit it.
2. Create `WowLoadingMorph.swift` with the content in section B. Never edit it.
3. Use it: WowLoadingMorph
...
## States
- idle · motion: label at rest in a frame already sized for the widest state; returning from any phase is a 160 ms cross-fade · haptic: none
- loading · motion: label blurs out (opacity 1→0, blur 0→4 pt) over 120 ms easeOut; system spinner (.small, 14 pt) fades in over 120 ms from +40 ms · haptic: none: the user just pressed and knows
- success · motion: outgoing content fades out over 100 ms; checkmark scales 0.6→1 on spring 0.28/0.72 while fading in over 120 ms · haptic: .success at +180 ms, as the glyph settles (full size at 152 ms, crest at 202 ms); dropped if the phase changes first
- failure · motion: same as success, with an xmark · haptic: .error at +0 ms, before any visual work: failure never waits
